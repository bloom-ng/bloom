...
class OnboardingController extends Controller
{
    public function store(Request $request)
    {
        ...
        // Verify if it's a working day (Monday - Friday)
        ...
        // Verify if the meeting is at least 30 minutes ahead
        $meetingDateTime = Carbon::parse($validated['date']);
        $now = Carbon::now();
        $minutesAhead = $now->diffInMinutes($meetingDateTime, false);

        if ($minutesAhead < 30) {
            return back()->with('action-fail', 'Sorry, meetings must be booked at least 30 minutes in advance. Please select a later time.');
        }
