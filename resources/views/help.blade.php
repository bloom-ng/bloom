      ...
      <div class="formsContainer">
        <div class="flex flex-col w-full h-fit-content gap-20 bg-zinc-800 p-8">
          ...
          <form class="text-black flex flex-col items-center gap-6 w-full font-extralight leading-10" action="/contact" method="POST">
            @csrf
            <input class="w-full px-4 py-4 bg-neutral-300 italic text-black font-extralight" name="subject" hidden type="text" value="Contact" placeholder="Subject" />
            <input class="w-full px-4 py-4 bg-neutral-300 italic text-black font-extralight" type="text" name="name" required placeholder="Name/Organization Name" />
        ...
        <div class="flex flex-col w-full h-fit-content gap-20 bg-zinc-800 p-8">
          ...
          <form class="flex flex-col items-center gap-6 w-full leading-10" action="/appointments" method="post">
            ...
            <input class="w-full px-4 py-4 bg-neutral-300 italic text-black font-extralight" type="number" name="requester_phone" required placeholder="Phone number" />
            <input class="w-full px-4 py-4 bg-neutral-300 italic text-black font-extralight" type="date"  name="start"  min="{{\Carbon\Carbon::now()->addDay()->format('Y-m-d').'T'.\Carbon\Carbon::now()->format('H:m')}}"  
            max="{{\Carbon\Carbon::now()->addMonths(2)->format('Y-m-d').'T'.\Carbon\Carbon::now()->addMonths(2)->format('h:m')}}"
            pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}" />
  
            {{-- <label for="duration">Duration</label> --}}
            <select class="w-full px-4 py-4 bg-neutral-300 italic text-black font-extralight" name="duration" id="duration" class="text-dark form-control border-0" style="background-color: #d7defc;">
            ...
            <button class="w-full bg-amber-500 text-black font-black py-4" type="submit">BOOK A MEETING</button>
